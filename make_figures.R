...
set.seed(42)
tier_boot <- ev %>%
  mutate(tier = factor(player_tier,
                       levels = c("starter (ESPN 12+)","flex (ESPN 5-12)",
                                  "bench (ESPN 0-5)","zero (ESPN 0)"))) %>%
  filter(!is.na(tier)) %>%
  select(tier, fantasy_ppr, model_pred, ensemble_pred, espn_proj)

boot_ci <- function(err, B = 1000) {
  means <- replicate(B, mean(sample(err, replace = TRUE)))
...
tier_summary <- tier_boot %>%
  group_by(tier) %>%
  summarise(
    Standalone_mae = mean(abs(fantasy_ppr - model_pred)),
    Standalone_lo  = boot_ci(abs(fantasy_ppr - model_pred))[1],
    Standalone_hi  = boot_ci(abs(fantasy_ppr - model_pred))[2],
    Ensemble_mae = mean(abs(fantasy_ppr - ensemble_pred)),
    Ensemble_lo  = boot_ci(abs(fantasy_ppr - ensemble_pred))[1],
    Ensemble_hi  = boot_ci(abs(fantasy_ppr - ensemble_pred))[2],
    ESPN_mae = mean(abs(fantasy_ppr - espn_proj)),
    ESPN_lo  = boot_ci(abs(fantasy_ppr - espn_proj))[1],
    ESPN_hi  = boot_ci(abs(fantasy_ppr - espn_proj))[2],
    .groups = "drop"
  ) %>%
  pivot_longer(-tier, names_to = c("Model",".value"), names_sep = "_")

p6 <- ggplot(tier_summary, aes(x = tier, y = mae, color = Model)) +
  geom_point(position = position_dodge(width = 0.55), size = 3) +
  geom_errorbar(aes(ymin = lo, ymax = hi),
                position = position_dodge(width = 0.55), width = 0.2, linewidth = 0.7) +
  scale_color_manual(values = model_colors) +
  labs(x = "Player tier (by ESPN projection)", y = "MAE (PPR points)",
