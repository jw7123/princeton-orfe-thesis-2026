...
# ============================================================
# Trains ONLY on player-weeks where the player took the field
# (played == 1). This removes thousands of zero-point rows from
# training, letting the model focus on distinguishing a 10-point
# week from a 15-point week instead of learning "inactive = 0".
# Post-processing still handles zeroing out inactive players.
# ============================================================
# Improvements over previous version:
#   1)  Per-position XGBoost models (RB, WR, TE trained separately)
#   2)  (Standalone: no ESPN blending — pure model predictions)
#       PLAYED-ONLY TRAINING: model only learns from active players
...
weekly <- weekly %>%
  ...
  left_join(def_fp_allowed_by_pos %>%
    select(season, week, defense_team, opp_position, def_fp_allowed_to_pos_avg3),
...
weekly_features <- weekly %>%
  ...
  select(
    gsis_id, season, week,
    lag1_fp, lag2_fp, lag3_fp, avg3_fp, avg5_fp, ewma_fp, n3_fp,
    season_avg_fp, season_games,
    fp_sd3, fp_sd5, fp_min3, fp_max3, fp_range3,
    lag_target_share, avg3_target_share, avg5_target_share, n3_target_share,
    lag_air_yards_share, avg3_air_yards_share,
    lag_rec_air_yards, avg3_rec_air_yards,
    lag_wopr, avg3_wopr, n3_wopr,
...
model_tbl <- injury_features %>%
  ...
  left_join(snap_features %>%
    select(gsis_id, season, week,
           offense_pct, lag1_offense_pct, avg3_offense_pct,
...
carry_forward_cols <- c(
  "lag1_fp", "avg3_fp", "avg5_fp", "ewma_fp", "season_avg_fp", "season_games",
  "fp_sd3", "fp_sd5",
  "lag_target_share", "avg3_target_share", "avg5_target_share",
  "lag_air_yards_share", "avg3_air_yards_share",
  "lag_rec_air_yards", "avg3_rec_air_yards",
  "lag_wopr", "avg3_wopr",
  "lag_rz_opps", "avg3_rz_opps",
  "lag1_offense_pct", "avg3_offense_pct", "lag1_was_active",
  "active_weeks_last_3", "consecutive_zero_snap_weeks"
)
...
xgb_feature_cols <- c(
  ...
  # Snap counts
  "lag1_offense_pct", "avg3_offense_pct",
...
score_one_week <- function(eval_season, eval_week, full_data,
  ...
  train <- full_data %>%
    filter(position %in% skill_pos,
           played == 1,  # PLAYED-ONLY: train exclusively on active players
           season_week_key(season, week) < cutoff_key)

  eval  <- full_data %>%
    filter(position %in% skill_pos,
           season == eval_season, week == eval_week)
    # NOTE: eval includes ALL players (played or not) — post-processing
    # handles zeroing out inactive players at prediction time
...
if (run_2023_2024_backtest) {
  ...
  if (run_feature_pruning) {
    ...
    init_train <- model_tbl %>%
      filter(position %in% skill_pos, season %in% c(2023, 2024), played == 1)
  ...
  if (run_quantile_models) {
    ...
    q_train <- model_tbl %>%
      filter(position %in% skill_pos, season == 2023, played == 1)
  ...
  write_csv(bt_predictions, "backtest_2023_2024_predictions.csv")
  write_csv(bt_weekly, "backtest_2023_2024_weekly_metrics.csv")
}
...
if (run_2025_eval) {
  ...
  if (run_hyperparam_tuning && is.null(tuned_params_2025)) {
    ...
    tune_data_2025 <- model_tbl %>%
      filter(position %in% skill_pos, season %in% backtest_seasons, played == 1)
  ...
  if (run_feature_pruning && is.null(pruned_feats_2025)) {
    ...
    init_2025 <- model_tbl %>% filter(position %in% skill_pos, season %in% backtest_seasons, played == 1)
  ...
  cat("\n-- 2025 Model vs ESPN (benchmark) (Overall) --\n")
  print(comp_overall)
  ...
  cat("\n-- 2025 Model vs ESPN (benchmark) (Played only) --\n")
  ...
  cat("\n-- Post-processing impact (raw vs adjusted vs ESPN) --\n")
  print(comp_postproc)
  ...
  write_csv(eval_2025_predictions, "eval_2025_predictions.csv")
  write_csv(comparison, "comparison_2025_model_vs_espn.csv")
  write_csv(comp_by_week, "comparison_2025_by_week.csv")
  write_csv(comp_by_pos, "comparison_2025_by_position.csv")
  write_csv(comp_by_tier, "comparison_2025_by_tier.csv")
  write_csv(as_tibble(all_importance), "feature_importance_by_position.csv")
}
...
if (run_hyperparam_tuning) {
  cat("\n=== Hyperparameter stability check ===\n")
  cat("Re-tuning on (2023-2024 + Weeks 1-9 of 2025) data:\n")
  midseason_data <- model_tbl %>%
    filter(position %in% skill_pos,
           (season %in% c(2023, 2024)) | (season == 2025 & week <= 9))
  midseason_result <- tune_xgb_params(midseason_data, n_trials = tuning_n_trials,
                                      include_pos_dummies = FALSE)
  cat("Midseason tune chose:\n")
  print(midseason_result$params)
  cat("Original tune chose:\n")
  print(tuned_params)
  cat("(If these are similar, retuning frequency does not matter much.)\n")
}